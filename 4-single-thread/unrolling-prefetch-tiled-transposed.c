    ...
    for (int i = 0; i < M; i += TILE_SIZE) {
        for (int k = 0; k < K; k += TILE_SIZE) {
            for (int j = 0; j < N; j += TILE_SIZE) {
                for (int ii = i; ii < i + TILE_SIZE && ii < M; ii += 4) {
                    for (int kk = k; kk < k + TILE_SIZE && kk < K; kk++) {
                        _mm_prefetch((char*)&A[(ii+4) * K + kk], _MM_HINT_T0);
                        ...
                        for (int jj = j; jj < j + TILE_SIZE && jj < N; jj += 16) {
                            ...
                            __m256 b0 = _mm256_loadu_ps(&B_transposed[jj * K + kk]);
                            __m256 b1 = _mm256_loadu_ps(&B_transposed[(jj+8) * K + kk]);

                            __m256 c00 = _mm256_loadu_ps(&C[ii * N + jj]);
                            __m256 c01 = _mm256_loadu_ps(&C[ii * N + jj + 8]);
                            __m256 c10 = _mm256_loadu_ps(&C[(ii+1) * N + jj]);
                            __m256 c11 = _mm256_loadu_ps(&C[(ii+1) * N + jj + 8]);
                            __m256 c20 = _mm256_loadu_ps(&C[(ii+2) * N + jj]);
                            __m256 c21 = _mm256_loadu_ps(&C[(ii+2) * N + jj + 8]);
                            __m256 c30 = _mm256_loadu_ps(&C[(ii+3) * N + jj]);
                            __m256 c31 = _mm256_loadu_ps(&C[(ii+3) * N + jj + 8]);
                            ...
                            _mm256_storeu_ps(&C[ii * N + jj], c00);
                            _mm256_storeu_ps(&C[ii * N + jj + 8], c01);
                            _mm256_storeu_ps(&C[(ii+1) * N + jj], c10);
                            _mm256_storeu_ps(&C[(ii+1) * N + jj + 8], c11);
                            _mm256_storeu_ps(&C[(ii+2) * N + jj], c20);
                            _mm256_storeu_ps(&C[(ii+2) * N + jj + 8], c21);
                            _mm256_storeu_ps(&C[(ii+3) * N + jj], c30);
