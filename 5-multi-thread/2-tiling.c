...
void matmul_blocked_2layer(float *A, float *B_transposed, float *C, int M, int N, int K) {
    for (int i_outer = 0; i_outer < M; i_outer += OUTER_BLOCK_SIZE) {
        for (int j_outer = 0; j_outer < N; j_outer += OUTER_BLOCK_SIZE) {
            for (int k_outer = 0; k_outer < K; k_outer += OUTER_BLOCK_SIZE) {
                int max_i_outer = (i_outer + OUTER_BLOCK_SIZE < M) ? i_outer + OUTER_BLOCK_SIZE : M;
                int max_j_outer = (j_outer + OUTER_BLOCK_SIZE < N) ? j_outer + OUTER_BLOCK_SIZE : N;
                int max_k_outer = (k_outer + OUTER_BLOCK_SIZE < K) ? k_outer + OUTER_BLOCK_SIZE : K;

                for (int i_inner = i_outer; i_inner < max_i_outer; i_inner += INNER_BLOCK_SIZE) {
                    for (int j_inner = j_outer; j_inner < max_j_outer; j_inner += INNER_BLOCK_SIZE) {
                        for (int k_inner = k_outer; k_inner < max_k_outer; k_inner += INNER_BLOCK_SIZE) {
                            int max_i = (i_inner + INNER_BLOCK_SIZE < max_i_outer) ? i_inner + INNER_BLOCK_SIZE : max_i_outer;
                            int max_j = (j_inner + INNER_BLOCK_SIZE < max_j_outer) ? j_inner + INNER_BLOCK_SIZE : max_j_outer;
                            int max_k = (k_inner + INNER_BLOCK_SIZE < max_k_outer) ? k_inner + INNER_BLOCK_SIZE : max_k_outer;

                            for (int i = i_inner; i < max_i - 3; i += 4) {
                                for (int k = k_inner; k < max_k; k++) {
                                    ...
                                    for (int j = j_inner; j < max_j - 15; j += 16) {
                            ...
                            for (int i = max_i - (max_i % 4); i < max_i; i++) {
                                for (int k = k_inner; k < max_k; k++) {
                                    ...
                                    for (int j = j_inner; j < max_j - 7; j += 8) {
...
int main() {
    ...
    for (int i = 0; i < num_sizes; i++) {
        ...
        for (int test = 0; test < TESTS; test++) {
            ...
            matmul_blocked_2layer(A, B_transposed, C, M, N, K);
